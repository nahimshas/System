...
| `tools/analysis/market_test.py` | **The decisive test: do our features beat the market?** Time-split logistic regression (market-price-only vs market+features), pure numpy. Aug 2026: MLB features WORSEN out-of-sample log-loss in all 3 markets = no incremental signal. Run this on ANY new market before betting it. |
| `tools/analysis/pattern_search.py` | Brute-force profitable-pattern hunt (ROI first, CLV ignored), with discovery/validation time split + multiple-comparisons discipline. Aug 2026: 28 segments, zero survivors. |
...
- **⚠️ MLB FULL-GAME HAS NO EXPLOITABLE EDGE (proven Aug 4 2026 — read before proposing model "improvements").** Five independent tests agree: budget CLV −0.65% (n=212); features worsen out-of-sample log-loss; when we disagree with the market the market is right and our error GROWS with our claimed edge; 28-segment pattern search found zero robust winners; and no sizing scheme helps because **nothing we log separates our winners from losers** (HIGH-confidence picks lose MORE — the label is inverted). Do NOT propose new constants/patterns/sizing for MLB full-game — that question is closed. New markets (F5, Liga MX, NFL) must each pass `market_test.py` + CLV ≥ 0 + positive ROI in both halves of a time split BEFORE real money; use `BUDGET_EXCLUDED_MARKETS` to keep them display-and-log-only meanwhile. Full detail: the Aug 4 entries in the calibration-audit memory.
- **⚠️ NOTIFICATION PIPELINE (Jul 2026) — fragile, breaks silently.** Before touching ANYTHING notification-related (worker.js, results_snapshot.yml notify step, notify_debrief.yml, health_report.yml notify step, KV keys), read the "🔔 NOTIFICATION PIPELINE — CANONICAL REFERENCE" section in the project memory file. Non-negotiables: (1) workflow GITHUB_TOKEN pushes DON'T trigger other workflows — `results_snapshot.yml` must keep sending its own notification (notify_debrief.yml only covers human/PAT pushes); (2) one notification/day via the `debrief_notify:{Pacific-date}` KV key — both writers check-then-write (dedupe); local docs republishes consume the day's slot unless notify_debrief is temporarily disabled; (3) the worker's EARLY debrief block must stay BEFORE the `picks:{date}` gate in `runCron()`; (4) worker deploys need the CF token to have Workers Scripts:Edit + KV:Edit AND `account_id` pinned in wrangler.toml — a failed deploy means worker code changes are silently NOT live; (5) nightly trigger is EXTERNAL (cron-job.org 10:46pm PT → workflow_dispatch); GitHub's own crons are delayed ~2-3h and serve only as backup sweeps.
