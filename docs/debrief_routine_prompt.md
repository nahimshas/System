# Nightly Debrief Routine — canonical prompt (git-based, Jul 2026)

> This is the CANONICAL REFERENCE COPY of the nightly debrief routine. The live
> routine is pasted manually into the Claude app (do not assume auto-sync) —
> when this file changes, the user must re-paste it into the `nightly-debrief`
> routine, re-inserting real credentials where the placeholders are.
> Never commit real tokens here.
>
> Routine platform requirements (learned Jul 1 2026): the routine's repository
> selector must include `nahimshas/System` and the account-level GitHub
> Integration connector must be installed — git pushes fail 403 otherwise.
>
> HISTORY: the original REST-API version of this routine (api.github.com
> contents/dispatch endpoints) was retired Jul 1 2026 — the routine sandbox's
> proxy blocks api.github.com (502) and site.api.espn.com (403), while plain
> git clone/fetch/push to github.com works. Every GitHub read/write below goes
> through a local git clone. api.cloudflare.com is ALSO proxy-blocked — the
> routine's own KV write usually fails (non-fatal); the redundant
> `notify_debrief.yml` workflow (fires on push of debrief_latest.html) is the
> KV path that actually works.
...
IMPORTANT — network note: this session's proxy intermittently blocks `api.github.com` (REST API) and `site.api.espn.com` with 502/403 errors, but plain `git clone`/`git fetch`/`git push` to `github.com` works reliably. Every step below reads and writes GitHub content through a local git clone instead of REST API calls — never fall back to `urllib`/`curl` calls against `api.github.com` for anything in this routine.
...
## Step 2 — Sync the local clone and read today's state file
...
def sync_repo():
    """Clone on first run, otherwise fetch + hard-reset to origin/main.
    Plain git push/fetch to github.com works reliably even when this
    session's proxy blocks api.github.com REST calls — every GitHub
    read/write in this routine goes through this local clone."""
    if not os.path.isdir(os.path.join(REPO_DIR, ".git")):
        subprocess.run(["git", "clone", REMOTE_URL, REPO_DIR], check=True, capture_output=True, text=True)
    else:
        subprocess.run(["git", "-C", REPO_DIR, "remote", "set-url", "origin", REMOTE_URL], check=True)
        subprocess.run(["git", "-C", REPO_DIR, "fetch", "origin", "main"], check=True, capture_output=True, text=True)
        subprocess.run(["git", "-C", REPO_DIR, "reset", "--hard", "origin/main"], check=True, capture_output=True, text=True)
    subprocess.run(["git", "-C", REPO_DIR, "config", "user.name", "debrief-agent"], check=True)
    subprocess.run(["git", "-C", REPO_DIR, "config", "user.email", "debrief-agent@users.noreply.github.com"], check=True)
...
def load_state(candidate):
    path = os.path.join(REPO_DIR, f"state/picks_{candidate}.json")
...
for candidate in [pacific_date, utc_date, yesterday_pacific]:
    ...
    if candidate_state and (candidate_state.get("singles") or candidate_state.get("singles_display")):
        today = candidate
...
NOTE (Jul 2026): fewer budget picks than 5 is NORMAL — the Jul 4 optimization
raised the budget entry floor to a 5% effective edge, so thin days produce
thin cards by design. Do not treat a 2-3 pick card as an error.
...
The workflow is triggered by a **git push**, not the `workflow_dispatch` REST endpoint — `api.github.com` is unreliable from this session, but `git push` to `github.com` is not. `results_snapshot.yml` has a `push: paths: state/.snapshot_trigger` trigger wired specifically for this.
...
# 1. Trigger the workflow via a push to state/.snapshot_trigger
sync_repo()

trigger_path = os.path.join(REPO_DIR, "state/.snapshot_trigger")
with open(trigger_path, "w") as f:
    f.write(datetime.now(timezone.utc).isoformat())

subprocess.run(["git", "-C", REPO_DIR, "add", "state/.snapshot_trigger"], check=True)
subprocess.run(["git", "-C", REPO_DIR, "commit", "-m", f"Trigger results snapshot: {today}"],
...
for attempt in range(3):
    ...
    subprocess.run(["git", "-C", REPO_DIR, "rebase", "origin/main"], check=True)
print(f"Results Snapshot trigger pushed: {pushed}")

# 2. Poll for the fresh snapshot (the workflow takes ~90-150s incl. CLV capture)
def get_local_snapshot():
    subprocess.run(["git", "-C", REPO_DIR, "fetch", "origin", "main"], check=True, capture_output=True, text=True)
    subprocess.run(["git", "-C", REPO_DIR, "reset", "--hard", "origin/main"], check=True, capture_output=True, text=True)
    path = os.path.join(REPO_DIR, "state/results_snapshot.json")
    try:
        with open(path) as f:
            return json.load(f)
    except Exception:
...
- MLB: Park Factor, ERA Trap, Injury Impact, Platoon Edge, Pitching Edge, Recent Form, Home Advantage, Pitcher Matchup, Schedule Load, Weather Factor, xFIP Projection, **Starter Edge** (use for the dog-with-better-starter validated pattern — any pick whose signals contain "Validated pattern").
...
NOTE (Jul 2026): injuries no longer move the model's probability (the market
already prices them) — an "injury impact" line in signals is roster CONTEXT.
Only tag "Injury Impact" as a signal when it is genuinely the pick's context,
and never describe injuries as the pick's driver in why_picked.
...
hist_path = os.path.join(REPO_DIR, "docs/debrief_history.json")
try:
...
subprocess.run(["git", "-C", REPO_DIR, "add", "docs/debrief_history.json"], check=True)
subprocess.run(["git", "-C", REPO_DIR, "commit", "-m", f"Debrief history: {today}"],
                capture_output=True, text=True)
...
for attempt in range(3):
    ...
    subprocess.run(["git", "-C", REPO_DIR, "fetch", "origin", "main"], check=True)
    subprocess.run(["git", "-C", REPO_DIR, "rebase", "origin/main"], check=True)
print(f"History updated, pushed: {pushed}")
```
...
## Step 6 — Publish HTML via the local clone
...
with open("/tmp/debrief_latest.html", encoding="utf-8") as f:
    html_content = f.read()

dest_path = os.path.join(REPO_DIR, "docs/debrief_latest.html")
with open(dest_path, "w", encoding="utf-8") as f:
    f.write(html_content)

subprocess.run(["git", "-C", REPO_DIR, "add", "docs/debrief_latest.html"], check=True)
subprocess.run(["git", "-C", REPO_DIR, "commit", "-m", f"Debrief: {today}"],
                capture_output=True, text=True)
...
for attempt in range(3):
    ...
    subprocess.run(["git", "-C", REPO_DIR, "fetch", "origin", "main"], check=True)
    subprocess.run(["git", "-C", REPO_DIR, "rebase", "origin/main"], check=True)
print(f"Published, pushed: {pushed}")
```
...
NOTE: api.cloudflare.com is often blocked by this session's proxy — if the
write fails, report it and move on (non-fatal): the `notify_debrief.yml`
workflow fires the same KV write automatically when debrief_latest.html is
pushed, and that path is verified working.
...
try:
    ...
    print(f"KV write error: {e} — non-fatal, notify_debrief.yml covers this")
```
...
- All GitHub reads and writes go through the local clone at /tmp/system_repo via git, never via api.github.com REST calls — that endpoint is unreliable from this session's network.
