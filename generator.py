...
def _now_pacific() -> datetime:
    ...
    # PDT: second Sunday March – first Sunday November
    # Simple check: months 3–10 use PDT, rest use PST
    if 3 <= now_utc.month <= 10:
        return now_utc.astimezone(PDT)
    return now_utc.astimezone(PST)
...
def build_report(
    ...
    return {
        "generated_at": _now_pacific().strftime("%B %d, %Y at %I:%M %p %Z"),
